...
import { LevelsRow } from "@/components/levels/levels-row.component"
import type { Level } from "@/types/module.type"

export default async function LevelsPage() {
  const levels = (await serverFetch(`http://localhost:3000/api/levels`, { next: { revalidate: 120 } })
      ...
      <main className="flex-1 lg:ml-64">
        <header className="bg-background sticky top-0 z-10 w-full border-b border-outline-variant/10">
          <div className="flex justify-between items-center px-12 py-4 max-w-[1440px] mx-5">
            <h1 className="text-[24px] font-[600] leading-[32px] text-primary">Levels</h1>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead className="bg-surface-container-low">
                  <tr>
                    {["Level", "Modules", "Created", ""].map((h) => (
                      <th
                        key={h}
                        className={`px-6 py-3 text-[12px] font-[500] leading-[16px] text-on-surface-variant uppercase tracking-wider ${h === "" ? "text-right" : ""}`}
                      >
                        {h}
                      </th>
                    ))}
