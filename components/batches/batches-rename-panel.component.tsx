...
interface BatchesRenamePanelProps {
  batch: Batch
  onClose: () => void
}

export function BatchesRenamePanel({ batch, onClose }: BatchesRenamePanelProps) {
  ...
  const {
    ...
    defaultValues: { batch_name: batch.batch_name },
  })
  ...
  async function onSubmit(data: RenameFormValues) {
    ...
    try {
      const res = await fetch(`/api/batches/${batch.id}`, {
        ...
        body: JSON.stringify({ batch_name: data.batch_name }),
      })

      if (!res.ok) {
        const body = await res.json().catch(() => null)
        setSubmitError(body?.error ?? "Something went wrong while renaming the batch.")
        return
      }
  ...
  return (
    <>
        ...
        <div className="flex-1 space-y-4 overflow-y-auto px-6 py-6">
          <div>
            ...
            <p className="mt-1 text-[12px] leading-[18px] text-on-surface-variant">
              Rename the batch. Its levels cannot be changed.
            </p>
          </div>
      ...
      <ConfirmDialog
        open={showConfirm}
        title="Save changes?"
        description={`Are you sure you want to rename "${batch.batch_name}"?`}
        confirmLabel="Save"
