  ...
  async function handleDelete() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setSubmitError(body?.error ?? "Something went wrong while removing the enrollments.")
        setIsDeleting(false)
        return
      }
      ...
      <div className="flex flex-1 flex-col">
        <div className="flex-1 space-y-4 overflow-y-auto px-6 py-6">
          <div className="flex items-start gap-3 rounded-xl border border-destructive/20 bg-destructive/10 px-4 py-3">
            ...
            <div>
              <p className="text-[14px] font-[600] leading-[20px] text-on-surface">Remove this student&apos;s enrollments?</p>
              <p className="mt-1 text-[13px] leading-[18px] text-on-surface-variant">
                This will unenroll {student.full_name} from all modules. This action cannot be undone.
              </p>
            </div>
          </div>
